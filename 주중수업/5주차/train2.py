...
# 하이퍼파라메터 선언 
# ctrl +d  ->  ctrl + shift 로 단어 단위 선택 가능
lr = 0.001
image_size = 28
num_classes = 10 
hidden_size = 500 
...
target_folder_name = max ([0]+[int(e) for e in os.listdir(results_folder)])+1  # 아무것도 없을때 처리를 위해서 0을 넣고 아니면 최대 값에서 +1을 해서 폴더를 만들어줌
targer_folder = os.path.join(results_folder,str(target_folder_name))
os.makedirs(targer_folder)
# 타켓 폴더 밑에 hparam 저장 (text의 형태로)
with open(os.path.join(targer_folder,'hparam.txt'),'w') as f:
    f.write(f'{lr =}\n')
